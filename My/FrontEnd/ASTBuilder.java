...
public class ASTBuilder extends MxllBaseListener {
	...
	@Override public void exitForStatement(MxllParser.ForStatementContext ctx) {
		rst.put(ctx, new ForStatement(new Location(ctx), getExpr(ctx.init), getExpr(ctx.cond),
									  getExpr(ctx.step), getStmt(ctx.statement()) ));
	}
	...
	@Override public void exitSubscriptExpression(MxllParser.SubscriptExpressionContext ctx) {
		Expression expr = getExpr(ctx.expression(0));
		if (!(ctx.expression(0) instanceof MxllParser.SubExpressionContext) && expr instanceof NewExpression)
			throw new SemanticError(new Location(ctx), "Invalid new expression");
		rst.put(ctx, new SubscriptNode(expr, getExpr(ctx.expression(1))));
	}
	...
	@Override public void exitNewExpression(MxllParser.NewExpressionContext ctx) {
		...
		int dimension = (ctx.getChildCount() - 2 - exprNum * 3) / 2 + exprNum;

		if (baseType instanceof ArrayType || baseType instanceof VoidType) {
			throw new SemanticError(new Location(ctx), "Invalid new expression");
		} else if (dimension == 0 && (baseType instanceof BoolType || baseType instanceof IntegerType)) {
			throw new SemanticError(new Location(ctx), "Invalid new expression");
		} else {
